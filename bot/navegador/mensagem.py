...
class Response:
    ...
    status: int
    headers: bot.estruturas.DictNormalizado
    body: Any

    def __init__ (self) -> None:
        self.headers = bot.estruturas.DictNormalizado()
...
class Request:
    ...
    def __init__ (self) -> None:
        self.url, self.headers = bot.http.Url(""), bot.estruturas.DictNormalizado()
...
class Mensagem:
    ...
    def parse_request (self, params: bot.formatos.Json) -> None:
        ...
        if headers := (params.headers or params.request.headers).obter(Any):
            self.request.headers = self.request.headers or bot.estruturas.DictNormalizado()
    ...
    def parse_response (self, params: bot.formatos.Json) -> None:
        ...
        if headers := (params.headers or params.response.headers).obter(Any):
            self.response.headers = self.response.headers or bot.estruturas.DictNormalizado()
            for header in headers:
